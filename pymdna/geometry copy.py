...
class ReferenceBase:
    ...
    def calculate_base_frame(self):
        ...
        b_L /= np.linalg.norm(b_L, axis=1, keepdims=True)  # Normalize b_L to have unit length

        # Calculate b_D using cross product of b_L and b_N
        b_D = np.cross(b_L, b_N, axis=1)
        b_D /= np.linalg.norm(b_D, axis=1, keepdims=True)  # Normalize b_D to have unit length
